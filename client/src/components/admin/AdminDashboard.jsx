...
function AdminDashboard() {
    ...
    async function getAllAuthors() {
        try {
            const token = await getToken();
            let res = await axios.get(`${API_URL}/admin-api/authors`, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            });
    ...
    async function blockUsers(email) {
      try {
          ...
          let res = await axios.put(`${API_URL}/admin-api/user/${email}`,
              { isActive: false });
    ...
    async function blockAuthors(email) {
      ...
      try{
        ...
        let res = await axios.put(`${API_URL}/admin-api/author/${email}`,
            { isActive: false });
        if (res.data.message === 'Your account is blocked or activated') {
...
//         try {
//             const token = await getToken();
//             const res = await axios.get(`${API_URL}/admin-api/authors`, {
//                 headers: { Authorization: `Bearer ${token}` }
//             });
//             if (res.data.message === 'all authors') {
//                 setStoreauthor(res.data.payload);
...
//     async function blockUsers(email) {
//         try {
//             let res = await axios.put(`${API_URL}/admin-api/user/${email}`, { isActive: false });
//             if (res.data.message === 'Your account is blocked or activated') {
//                 setStoreUser(storeuser.map(user => user.email === email ? { ...user, isActive: false } : user));
//                 getAllUsers();
//             }
...
//     async function activateUsers(email) {
//         try {
//             let res = await axios.put(`${API_URL}/admin-api/user/${email}`, { isActive: true });
//             if (res.data.message === 'Your account is blocked or activated') {
//                 setStoreUser(storeuser.map(user => user.email === email ? { ...user, isActive: true } : user));
//                 getAllUsers();
//             }
...
//     async function blockAuthors(email) {
//         try {
//             let res = await axios.put(`${API_URL}/admin-api/author/${email}`, { isActive: false });
//             if (res.data.message === 'Your account is blocked or activated') {
//                 setStoreauthor(storeauthor.map(author => author.email === email ? { ...author, isActive: false } : author));
//                 getAllAuthors();
//             }
...
//             setError('Failed to block author');
//         }
//     }

//     async function activateAuthors(email) {
//         try {
//             let res = await axios.put(`${API_URL}/admin-api/author/${email}`, { isActive: true });
//             if (res.data.message === 'Your account is blocked or activated') {
//                 setStoreauthor(storeauthor.map(author => author.email === email ? { ...author, isActive: true } : author));
//                 getAllAuthors();
//             }
//         } catch (error) {
//             console.error('Error activating author:', error);
